...
function NetworkHandler(improvedStorage) {
	...
	this.requestNewExtToken = async () => {
		...
		if (!response || response["type"] != "success" || !response["data"]) {
			console.log("Failed to get new ext token for " + this.type + " session. Response:", response);
			return (null);
		}
		else {
			console.log("Fetched a new ext token for " + this.type + " session");
		}
	...
	this.request = async (url, method = "GET", data = null, headers = {}) => {
		console.log("Fetching URL " + url + " in " + this.type + " context");
